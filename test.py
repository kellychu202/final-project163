...
def test_genre():
    ...
    assert_equals(14, genre.genre_count(small_movies, 'Netflix'))
    print()
    assert_equals(1, genre.genre_count(small_movies, 'Hulu'))
    print()
    assert_equals(4, genre.genre_count(small_movies, 'Disney+'))
    print()
    assert_equals(6, genre.genre_count(small_movies, 'Prime Video'))
    print()

def test_unique_list():
    small_movies = pd.read_csv('data/small_movies.csv')
    assert_equals(['Action', 'Adventure', 'Sci-Fi', 'Thriller', 'Comedy', 'Western', 'Animation', \
'Family', 'Biography', 'Drama', 'Music', 'War', 'Crime', 'Documentary'], genre.unquie_list(small_movies, 'Netflix'))
    assert_equals(['Documentary'], genre.unquie_list(small_movies, 'Hulu'))
    assert_equals(['Action', 'Adventure', 'Drama', 'Family'], genre.unquie_list(small_movies, 'Disney+'))
    assert_equals(['Western', 'Biography', 'Drama', 'Music', 'War', 'Comedy'], genre.unquie_list(small_movies, 'Prime Video'))
